...
const navItem = "whitespace-nowrap py-1 text-nano uppercase tracking-nav";
...
export default function Nav() {
  ...
  const revealOnScroll = pathname === "/the-house";
  ...
  return (
    <header
      ...
      className={`sticky top-0 z-50 border-b border-brass/70 bg-limewash/90 backdrop-blur-[2px] transition-opacity duration-300 ease-out focus-within:opacity-100 ${
    ...
    >
      ...
      >
        <Link
          ...
          className="py-1 text-ink transition-colors duration-300 hover:text-brass"
        >
          <Wordmark />
        </Link>
          ...
          {links.map((link, i) => {
            ...
            return (
              <li key={link.href} className="flex items-center gap-2 md:gap-3">
                {i > 0 && (
                  <span aria-hidden="true" className="text-nano text-brass">
                    ·
                  </span>
                )}
                {/*
                ...
                */}
                {current ? (
                  ...
                  <Link
                    href={link.href}
                    className={`${navItem} text-ink transition-colors duration-300 hover:text-brass`}
                  >
                    {link.label}
                  </Link>
                )}
